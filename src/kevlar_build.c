...
size_t kevlar_count_files_in_folder(const char *folder_path,
                                    const char *filetype)
{
...
void kevlar_copy_assets(const char *src, const char *dest)
{
    ...
    while ((in_dir_obj = readdir(in_dir_stream))) {
        ...
        snprintf(out_file_path, CONFIG_MAX_PATH_SIZE, "%s/%s", dest,
                 in_dir_obj->d_name);

        char in_file_path[CONFIG_MAX_PATH_SIZE];
        snprintf(in_file_path, CONFIG_MAX_PATH_SIZE, "%s/%s", src,
                 in_dir_obj->d_name);
...
void kevlar_parse_md_from_folder(char *folder_path, char *dist_path,
                                 char *template_path, ListingItem *itemsList)
{
    ...
    while ((dir_item = readdir(folder_buf)) != NULL) {
        ...
        kevlar_get_opt_from_config(in_fp, "title",
                                   itemsList[post_count].lTitle);
        ...
        kevlar_parse_post_from_template(out_fp, postTemplatePath,
                                        "./config.ini", &itemsList[post_count]);
        fclose(out_file);
        post_count++;
    }
}
...
void kevlar_handle_build_command(const char *file_path)
{
    ...
    kevlar_generate_index_from_template("./dist/index.html", themePath,
                                        "./config.ini", itemsList, md_count);

    free(itemsList);

    // kevlar_build_template(kev_config.configIndexPath, "./dist/index.html",
    // &kev_config); kevlar_ok("Built the site using theme %s at
