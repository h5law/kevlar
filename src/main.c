...
enum COMMANDS { cmdHelp, cmdNew, cmdBuild, cmdNewPost, cmdRssFeed, cmdTail };

int argtype(char arg[])
{
    char param[][MAX_CMD_SIZE] = {"help", "new", "build", "new-post", "rss"};
...
void kevlar_usage_exit(const char *origin)
{
    ...
    printf("  rss        -- create a new rss feed for the site\n");
    exit(0);
}
