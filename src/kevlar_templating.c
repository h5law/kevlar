...
void kevlar_parse_template_str_token(char line[], char substr[], char content[],
                                     char *target)
{
    char tail[TEMPLATE_MAX_TAG_SIZE];
    strcpy(tail, strrchr(strstr(line, substr), '-') + 1);
...
void kevlar_parse_inline_tag(char *line, char *template_file_path, char *tag,
                             FILE *out_buf)
{
...
void kevlar_parse_str_inline_tag(char *line, char *template_file_path,
                                 char *tag, char *output)
{
    char stylesheet_path[CONFIG_MAX_PATH_SIZE];
    char stylesheet[TEMPLATE_MAX_FILE_SIZE] = "";
...
void kevlar_parse_header_and_footer(FILE *header_buf, char *output,
                                    ...
                                    char *config_file_path)
{
    ...
    while (fgets(line, TEMPLATE_MAX_LINE_SIZE, header_buf) != NULL) {
        if (strstr(line, "--STYLE")) {
            ...
            kevlar_parse_str_inline_tag(line, template_folder, "script",
                                        output);
            continue;
        } else if (strstr(line, "--")) {
...
void kevlar_parse_header(FILE *output_buf, char *template_folder,
                         char *config_file_path)
{
    ...
    if (!header_buf)
        kevlar_err("couldn't open %s perhaps your theme is invalid",
                   header_path);
    kevlar_parse_header_and_footer(header_buf, output, template_folder,
                                   config_file_path);
    fprintf(output_buf, "%s", output);
}

void kevlar_parse_footer(FILE *output_buf, char *template_folder,
                         char *config_file_path)
{
    ...
    if (!footer_buf)
        kevlar_err("couldn't open %s perhaps your theme is invalid",
                   footer_path);
    kevlar_parse_header_and_footer(footer_buf, output, template_folder,
                                   config_file_path);
    fprintf(output_buf, "%s", output);
}

void kevlar_generate_listing(char *entry_template_file_path,
                             char *config_file_path, ListingItem *itemsList,
                             char *target, int size)
{
    ...
    for (int i = 0; i < size; i++) {
        while (fgets(line, TEMPLATE_MAX_LINE_SIZE, template)) {
            if (strstr(line, "--DATE--")) {
                kevlar_parse_template_str_token(line, "--DATE--",
                                                itemsList[i].lDate, target);
            } else if (strstr(line, "--TITLE--")) {
                kevlar_parse_template_str_token(line, "--TITLE--",
                                                itemsList[i].lTitle, target);
            } else if (strstr(line, "--PATH--")) {
                kevlar_parse_template_str_token(
                        line, "--PATH--", strrchr(itemsList[i].lPath, '/') + 1,
                        target);
            } else if (strstr(line, "--CONTENT--")) {
                kevlar_parse_template_str_token(line, "--CONTENT--",
                                                itemsList[i].lContent, target);
            } else if (strstr(line, "--")) {
                char token[CONFIG_MAX_PATH_SIZE];
                strcpy(token, strstr(line, "--") + 2);
                *strstr(token, "--") = '\0';
...
void kevlar_parse_post_from_template(char *html_file_path,
                                     ...
                                     char *config_file_path, ListingItem *item)
{
    ...
    *strrchr(template_folder, '/') = '\0';
    ...
    while (fgets(line, TEMPLATE_MAX_LINE_SIZE, post_file) != NULL) {
        if (strstr(line, "--DATE--")) {
            kevlar_parse_template_token(line, "--DATE--", item->lDate,
                                        html_file);
        } else if (strstr(line, "--TITLE--")) {
            kevlar_parse_template_token(line, "--TITLE--", item->lTitle,
                                        html_file);
        } else if (strstr(line, "--HEADER--") || strstr(line, "--FOOTER--")) {
            kevlar_parse_header(html_file, template_folder, config_file_path);
        } else if (strstr(line, "--CONTENT--")) {
            kevlar_parse_template_token(line, "--CONTENT--", item->lContent,
                                        html_file);
...
void kevlar_generate_index_from_template(char        *dist_path,
                                         char        *template_folder_path,
                                         char        *config_path,
                                         ListingItem *itemsList, int size)
{
    char entry_template_path[CONFIG_MAX_PATH_SIZE];
    ...
    kevlar_generate_listing(entry_template_path, config_path, itemsList, output,
                            size);
    ...
    while (fgets(line, TEMPLATE_MAX_LINE_SIZE, template) != NULL) {
        if (strstr(line, "--STYLE")) {
            kevlar_parse_inline_tag(line, template_folder_path, "style",
                                    html_file);
            continue;
        } else if (strstr(line, "--LISTING--")) {
            kevlar_parse_template_token(line, "--LISTING--", output, html_file);
        } else if (strstr(line, "--SCRIPT")) {
            kevlar_parse_inline_tag(line, template_folder_path, "script",
                                    html_file);
            continue;
