...
void md_handleText(char input[RST_LINE_LENGTH], char output[RST_LINE_LENGTH])
{
    ...
    for (int i = 0; input[i] != '\0'; i++) {
        if (input[i] == '*') {
            ...
            if (input[i - 1] == '!') {
                snprintf(linkStr, RST_LINE_LENGTH, "<img src=\"%s\">%s</img>",
                         linkLink + 1, linkText);
                utl_truncateLast(output);
            } else {
                snprintf(linkStr, RST_LINE_LENGTH, "<a href=\"%s\">%s</a>",
                         linkLink + 1, linkText);
            }
...
void md_parse(char *in_file_path, char *out_file_path, int offset)
{
    ...
    if ((md_infile = fopen(in_file_path, "r")) == NULL) {
        kevlar_err("[%s] the file \"%s\" doesn't exist.", __FILE__,
                   in_file_path);
    }
    ...
    for (long currentLine = offset; currentLine < fileLength; currentLine++) {
        ...
        default:
            ...
            if (isdigit(file[currentLine][0])) {
                ...
                md_is_single_para(file, currentLine)
                        ? md_handle_single_para(file, currentLine)
                        : md_handle_para(file, currentLine);
            }
        }
    }
