...
void kevlar_generate_new_post(const char *folder_path, char *post_title)
{
    ...
    if (!md_file_buf)
        kevlar_err("something went wrong while generating %s", file_name);
...
void kevlar_generate_new_skeleton(KevlarSkeleton *skeleton)
{
    ...
    strcat(clone_git_command, "/k3vl4r");
    ...
    if (system("git --version >/dev/null 2>&1") != 0) {
        kevlar_warn(
                "couldn't find git on your system; continuing without a theme");
    } else {
        kevlar_ok("cloning theme into %s", skeleton->skel_template_folder_path);
        if (system(clone_git_command) == -1) {
            kevlar_warn("the command `%s` failed; continuing without a theme",
                        clone_git_command);
        };
    }
#endif
    ...
    *(strchr(default_md_file_path, '/')) = '\0';
    kevlar_ok(
            "Skeleton was setup; you can now "
...
void kevlar_handle_new_command(char folder_path[CONFIG_MAX_PATH_SIZE])
{
    ...
    case folderNonEmpty:
        kevlar_err("folder \"%s\" already exists and is not empty!",
                   folder_path);
        break;
    case folderNull:
